...
NX = 32   #number of sites 64
NP = 32   #number of particle 64
NSAMPLE = 1024  #number of sample
STEP = 1000

J = -1.0  # hopping strength
U = 3.0   # on-site interaction energy
...
for counter in range(STEP):
    ...
    if counter % 1 == 0:
        # with open(r'/home/huang/pyvenv/Program/NQS/BHM/MI_s_32_b_32/eloc_U_1.0_V_1.0_R_0.txt', 'a+') as tf:
        #     tf.write(str(counter))
        #     tf.write(' ')
        #     tf.write(str(eloc.mean()))
        #     tf.write('\n')
        #     tf.close()
        with open(r'/home/huang/pyvenv/Program/NQS/BHM/s_32_b_32/V_1_U_3_R/U_3.0_V_1.0_R_0.txt', 'a+') as tf:
            tf.write(str(counter))
            tf.write(' ')
